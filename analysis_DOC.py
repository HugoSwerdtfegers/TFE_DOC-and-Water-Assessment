...
parser.add_argument('-sc', '--scenario', help='Scenario', 
                        type=str, default="reference", choices=['reference','optimistic','conservative', 'comparison', 'variable_DOC'])


args = parser.parse_args()
scenario = args.scenario

if scenario == "variable_DOC":
    years = 1
    ...
    filename = "results_DOC/variable_DOC.json"
    dico = {}
    ...
    wind1 = (np.sum([e.solution.elements.WIND_PLANTS.objectives.unnamed[0], e.solution.elements.WIND_PLANTS.objectives.unnamed[1]]))/(5*10)
    hydrogen1 = (e.solution.elements.ELECTROLYSIS_PLANTS.objectives.unnamed[0] + e.solution.elements.ELECTROLYSIS_PLANTS.objectives.unnamed[1] + e.solution.elements.HYDROGEN_STORAGE.objectives.unnamed[0] + e.solution.elements.HYDROGEN_STORAGE.objectives.unnamed[1])/(5*10)
    battery1 = ( np.sum([e.solution.elements.BATTERY_STORAGE.objectives.unnamed[0],e.solution.elements.BATTERY_STORAGE.objectives.unnamed[1]]))/(5*10)
    solar1 = (e.solution.elements.SOLAR_PV_PLANTS.objectives.unnamed[0] + e.solution.elements.SOLAR_PV_PLANTS.objectives.unnamed[1])/(5*10)
    methanation1 = (e.solution.elements.METHANE_LIQUEFACTION_PLANTS.objectives.unnamed[0] + e.solution.elements.METHANE_LIQUEFACTION_PLANTS.objectives.unnamed[1] + e.solution.elements.LIQUEFIED_METHANE_REGASIFICATION.objectives.unnamed[0] + e.solution.elements.LIQUEFIED_METHANE_REGASIFICATION.objectives.unnamed[1] + e.solution.elements.METHANATION_PLANTS.objectives.unnamed[0] + e.solution.elements.METHANATION_PLANTS.objectives.unnamed[1] + e.solution.elements.LIQUEFIED_METHANE_CARRIERS.objectives.unnamed[0] + e.solution.elements.LIQUEFIED_METHANE_CARRIERS.objectives.unnamed[1] + e.solution.elements.LIQUEFIED_METHANE_STORAGE_DESTINATION.objectives.unnamed[0] + e.solution.elements.LIQUEFIED_METHANE_STORAGE_DESTINATION.objectives.unnamed[1] + e.solution.elements.LIQUEFIED_METHANE_STORAGE_HUB.objectives.unnamed[0] + e.solution.elements.LIQUEFIED_METHANE_STORAGE_HUB.objectives.unnamed[1])/(5*10)
    CO21 = (e.solution.elements.DIRECT_OCEAN_CAPTURE_PLANTS.objectives.unnamed[0] + e.solution.elements.DIRECT_OCEAN_CAPTURE_PLANTS.objectives.unnamed[1] + e.solution.elements.CARBON_DIOXIDE_STORAGE.objectives.unnamed[0] + e.solution.elements.CARBON_DIOXIDE_STORAGE.objectives.unnamed[1] + e.solution.elements.PIPELINE.objectives.named.pipe_cost_fix + e.solution.elements.PIPELINE.objectives.named.pipe_cost_var )/(5*10)
    Others1 = (e.solution.elements.HVDC.objectives.unnamed[0] + e.solution.elements.HVDC.objectives.unnamed[1]  + e.solution.elements.DESALINATION_PLANTS.objectives.unnamed[0] + e.solution.elements.WATER_STORAGE.objectives.unnamed[0])/(5*10)


  
    colors = ['#BFC9CA', '#D35400', '#F5B041', '#F4D03F', '#ABEBC6', '#5DADE2', '#2ECC71']
    categories = ['Variable DOC hub', 'Constant DOC hub']
    ...
    plt.savefig('images_DOC/cost_comparison_variable_DOC.png', dpi=150, bbox_inches='tight')


    plt.figure()
    categories = ['Constant DOC hub', 'Variable DOC hub']
    ...
    plt.savefig('images_DOC/capacity_comparison_variable_DOC.png', dpi=150, bbox_inches='tight')
